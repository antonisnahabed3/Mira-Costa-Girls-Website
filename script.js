...
const calendarEvents = {
  ...
  "2026-10-23": ["Training"],
  "2026-10-24": ["Training"],
  "2026-10-26": ["Training", "VT Weights 6-7am"],
  "2026-10-27": ["Training"],
  "2026-10-28": ["Training"],
  "2026-10-29": ["Training", "VT Weights 6-7am"],
  "2026-10-30": ["Training"],
  "2026-10-31": ["TBD"],
  "2026-11-01": ["Training", "VT Weights 6-7am"],
  "2026-11-02": ["Training"],
  "2026-11-03": ["Training"],
  "2026-11-04": ["Training", "VT Weights 6-7am"],
  "2026-11-05": ["Training"],
  "2026-11-06": ["TBD"],
  "2026-11-08": ["Girls First Contest", "Training", "VT Weights 6-7am"],
  "2026-11-09": ["Training"],
  "2026-11-10": ["Training", "Veterans Day"],
  "2026-11-11": ["Training", "VT Weights 6-7am"],
  "2026-11-12": ["Training"],
  "2026-11-13": ["TBD"],
  "2026-11-15": ["Training"],
...
const rosterPhoto = document.querySelector("[data-roster-photo]");
const rosterPhotoImg = document.querySelector("[data-roster-photo-img]");
const gradYearSelect = document.querySelector("[data-grad-year]");
...
const activeRosters = rosters;
const activeResults = results;
const activeSeasonSummaries = seasonSummaries;
let activeCalendarMonths = calendarMonths;
let activeCalendarEvents = calendarEvents;
...
if (heroSlides.length > 1 && !prefersReducedMotion.matches) {
  ...
  let heroSlideIndex = Math.floor(Math.random() * heroSlides.length);
  let heroSlideQueue = shuffleSlides(heroSlides.map((_, index) => index).filter((index) => index !== heroSlideIndex));
  ...
  window.setInterval(() => {
    ...
    if (!heroSlideQueue.length) {
      heroSlideQueue = shuffleSlides(heroSlides.map((_, index) => index).filter((index) => index !== heroSlideIndex));
    }
    heroSlideIndex = heroSlideQueue.shift();
...
const fillSelect = (select, options, labels = {}) => {
  select.innerHTML = options.map((option) => `<option value="${option}">${labels[option] || option}</option>`).join("");
};

const getGraduationYearOptions = (date = new Date()) => {
  const firstYear = date.getFullYear() + 1;
  return Array.from({ length: 6 }, (_, index) => String(firstYear + index));
};

const emptyRosterSeason = () => ({ Varsity: [], JV: [], FS: [] });
const rosterTeamLabels = { Varsity: "Varsity", JV: "Junior Varsity", FS: "Frosh/Soph" };
const rosterTeamSlugs = { Varsity: "varsity", JV: "jv", FS: "fs" };

const getRosterSeasonForDate = (date = new Date()) => {
...
const parseRosterStartYear = (season) => Number.parseInt(season.split("/")[0], 10);

const orderedRosterYears = (availableYears, currentSeason) => {
  const ordered = [...availableYears].sort((a, b) => parseRosterStartYear(b) - parseRosterStartYear(a));
  return [currentSeason, ...ordered.filter((season) => season !== currentSeason)];
};

const getRosterPhotoSrc = (season, team) => {
  const [startYear, endYear] = season.split("/");
  const teamSlug = rosterTeamSlugs[team];

  if (!startYear || !endYear || !teamSlug) return "";

  return `assets/${teamSlug}-${startYear.slice(-2)}-${endYear.slice(-2)}-photo.jpg`;
};

const updateRosterPhoto = (season, team) => {
  if (!rosterPhoto || !rosterPhotoImg) return;

  const photoSrc = getRosterPhotoSrc(season, team);
  const teamName = rosterTeamLabels[team] || team;
  rosterPhoto.hidden = true;
  rosterPhotoImg.removeAttribute("src");

  if (!photoSrc) return;

  rosterPhotoImg.onload = () => {
    rosterPhoto.hidden = false;
  };
  rosterPhotoImg.onerror = () => {
    rosterPhoto.hidden = true;
  };
  rosterPhotoImg.alt = `${season} Mira Costa Girls Water Polo ${teamName} team photo`;
  rosterPhotoImg.src = photoSrc;
};
...
const renderRoster = () => {
  ...
  updateRosterPhoto(season, team);
  rosterTitle.textContent = `${season} ${teamName}`;
...
const syncRosterTeams = () => {
  fillSelect(rosterTeam, Object.keys(activeRosters[rosterYear.value]), rosterTeamLabels);
  renderRoster();
};

if (rosterYear && rosterTeam) {
  const currentRosterSeason = getRosterSeasonForDate();
  activeRosters[currentRosterSeason] ||= emptyRosterSeason();
  fillSelect(rosterYear, orderedRosterYears(Object.keys(activeRosters), currentRosterSeason));
  rosterYear.value = currentRosterSeason;
  syncRosterTeams();
...
if (gradYearSelect) {
  fillSelect(gradYearSelect, getGraduationYearOptions());
}
...
const loadProgramOverviewFromDoc = async () => {
  if (!overviewDocument || !overviewStatus) return;
...
if (overviewDocument && overviewStatus) {
